...
- 2026-07-28 05:00 UTC:**全量语料同步完成**——168,431/168,431 文件与 R2 对象数精确一致,2.3TiB,rclone 零错误,最新落盘文件抽检 3/3 LOAD_OK,v2 cache 已自动删除(下次训练启动全量重扫)。剩余空间 12T。canonical 全量 Stage A run(主 YAML)自此可随时启动,等用户拍板。

- 2026-07-28 晨:cadence-2000 追踪 16/16 零失败(pilot@2000 + slice512@2000 × r4 切换协议)。pilot 斜率演化 −0.30→−0.09,首现优于 off 基线信号;slice512@2000 贴基线零病理。站点 r4+ 两组卡上线。

- 2026-07-28 午后:**Stage A 双轨训练完成**——slice512(5693)4000/4000 EXIT_CODE=0(18h36m);pilot(5694)4000/4000 训练完整、ckpt-4000 落盘后进程组拆除竞态 SIGABRT(收尾噪声,非训练失败)。双 ckpt-4000 结构不变量全验证(LoRA 814 零泄漏 / partial 84 / query_state 缺席 / 权重全有限)。终态验收战役启动:r3+r4 双协议 × 双 ckpt@4000 × 8 case = 32 作业(5791-5822)四节点 30 卡并发。终态 loss CSV:results/stageA_*_loss_final.csv。

- 2026-07-28 傍晚:**Stage A 终态验收 PASS**——32/32 零失败,双协议全臂对照:pilot 谱系切换协议斜率单调收敛至 −0.002(零净漂移,基线 −0.343),静态协议基线重合;slice512@4000 静态平价、切换欠校准(−0.624)。判定 `logs/research/stage-a-terminal-verdict-2026-07-28.md`;Stage B 选基推荐 pilot@4000。站点两页 96 卡全指标就位。全量 run 5823(168k × 12k 步,16 卡)并行推进中。

- 2026-07-28 深夜:P2 r5 8 分钟长时域收口——10/10 零失败(连续第四轮)。判定 `logs/research/p2-longhorizon-verdict-r5-2026-07-28.md`:8min 尺度漂移形态翻转为中后段过饱和;off 臂 2/5 单调失控(→200),记忆臂斜率减半(+0.112 vs +0.219)且偏移全部末段回收——记忆=阻尼+回复力,尚非完全稳定器(Stage B 目标)。347 次写入状态机压力测试通过。站点 106 卡。

- 2026-07-29:全量 run(5823,168k × 12k 步,16 卡双节点)全程零错误推进至 11,691/12000,24 个 ckpt 全部结构验证通过(含多节点保存路径首验)。三轮 cadence 追踪均 8/8 或 5/5 零失败:**full@4000**(0.76ep,切换斜率 −0.193 优于基线与 slice512@4000 → "多样性可部分替代复读",判定附录 2)、**full@8000**(1.52ep,−0.263,未复现 pilot 式单调归零,附录 3)、**full@8000 × 8min**(+0.297,3/5 失控至 232,**劣于基线**,r5 判定附录)⇒ Stage B 选基回到 pilot@4000 唯一推荐。站点生成器改为自动收录 `p2-rep50-*`(3db0e60),现 127 卡两页。ckpt 质量判定体系与盲区入 findings 2026-07-29 条目。@12000 终态评测已用 Slurm 依赖链排好(5872-5884 评测 + 5885 metrics/建站,`afterany:5823`),脱离会话自动执行。

- 2026-07-29 深夜:**全量 run 5823 干净收官**(`COMPLETED / 0:0`,28h07m,12000/12000,末步 loss 0.0886、grad_norm 6.1e-4;ckpt-12000 不变量全过),依赖链 5872-5885 自动派发,切换协议 8 支已完成、8min 5 支在跑。**抗漂移新工作线开启**(用户观测 3min 起漂移 + 记忆无预防作用):三路研究(FramePack 取证 / 插桩点 file:line 映射 / r5 onset 细算)+ 实现 `HistoryProjector`(renorm/codebook/quantize + α 旋钮,`helios/utils/utils_base.py`)、pipeline 插桩(`pipeline_helios.py:1412-1427`)、驱动 CLI+manifest+效应量护栏、码本拟合脚本与 k128/k256 产物。对抗审查(refute 型)报 3 缺陷全部修复:效应量计数器(num_changed/max_abs_delta,防静默 no-op)、结构性 padding 判定(防误伤近常量帧)、dry-run digest 统一。测试 95→98 项全绿(新增 16 项投影单测,其中一项把"均匀量化能除偏"的错误推理钉成回归护栏)。设计契约 `docs/specs/2026-07-29-history-projection-design.md`(含 wave-1 九支筛选矩阵与门槛)。集群侧:pinghe 占满 c-node03/04/06/07 而 Slurm 报 idle ⇒ 两个 launcher 加属主守卫(EXIT_CODE=44)。

- 2026-07-30:抗漂移工作线成型并发出三个战役。**实现**:`HistoryProjector`(renorm/codebook/quantize + α 旋钮,`helios/utils/utils_base.py`)、pipeline 插桩(`pipeline_helios.py:1412-1427`,tier-split 后、KV cache 前)、驱动 CLI/manifest/效应量护栏、码本拟合脚本 + k128/k256 产物、两个 launcher 参数化(投影参数 / rep50 case 窗口,默认值保证既有战役逐字节不变)、判读 CLI `scripts/evaluation/drift_onset_report.py`(验收 = 精确复现已发布 onset/回收比数字)。**验证**:98 tests OK;对抗审查 3 缺陷全修(效应量计数器、结构性 padding 判定、dry-run digest 统一);GPU 冒烟 2/2 `EXIT_CODE=0` 且计数器与结构推算逐项对账(4/8 与 7/8 应用、30/152 padding、改变元素精确等于有效元素数)。**战役**:wave-1(5888-5896)、wave-1b 强度/K 扫描(5901-5908)、held-out 泛化 rep50 case 8-15 × 三臂(5911-5934),各带 metrics 尾节点,全部 `afterany` 链式脱离会话。**集群**:full@12000 依赖链 13/13 零失败(8 切换 + 5 × 8min),metrics 在算;按用户明确指令 sudo scancel yuheng 5843/5850(各已跑 50:08)释放 c-node05,mc-node01 被其排队作业 5858 接走;pinghe 占满 c-node03/04/06/07 而 Slurm 报 idle ⇒ launcher 加属主守卫(EXIT_CODE=44)。**Stage B 审计:LARGE GAP** —— 数据集/可微状态原语/capture API 三块就绪,缺 trainer unroll 循环;且 `train_helios.py:1575-1607` 的写入分支要求 `not memory_tf_unroll`,今天开 U=4 会训出 `memory_tokens=None` 的无记忆 ckpt(训练侧静默 no-op),故未发训练。概念台账 `agent-research/2026-07-30-history-discretization-anti-drift.md`。

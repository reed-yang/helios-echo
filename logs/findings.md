...
## 2026-07-30:wave-1b 判读 —— 免训练历史投影止漂成立,renorm 胜出、码本代价可测

**结论:renorm 在 8 分钟协议上把两个失控 case 的漂移斜率从 +0.51/+0.49 压到 ~0(峰值 243/244 → 80/131),同时保住 88–98% 的运动量;码本同样压平斜率但损失 31–46% 运动量,其"成功"被动态退化混淆。记忆从未达到止漂(最好只减半斜率),历史通路干预达到了。**

协议:8min(349 sections),off 骨干臂(无记忆),case 2/3 = 基线两个单调失控 case,种子对位。n=2/配置 ⇒ 筛选级证据。

| 臂 | slope(c2/c3) | 峰值饱和度 | Q1→Q4(c2) | 运动量 vs 基线 |
|---|---|---|---|---|
| off 基线 | **+0.514 / +0.491** | 243 / 244 | 52→108→128→193 | 100% |
| pilot@4000(记忆) | +0.204 / +0.011 | — | — | 102% / 94% |
| **renorm α=0.5** | **−0.006 / −0.052** | **80 / 131** | 48→50→46→47 | **98% / 88%** |
| renorm α=0.25 | −0.015 / −0.075 | 85 / 92 | 52→49→47→47 | 98% / 79% |
| cbk256 α=0.5 | −0.012 / +0.022 | 80 / 94 | 50→41→40→47 | **69% / 54%** |
| cbk128 α=1.0 | −0.014 / −0.013 | 92 / 88 | 44→49→42→45 | **55% / 60%** |

1. **不是"冻住所以不漂"**:零冻结,Q1 起点与基线相当(48–63 vs 52–68),发散被掐断而非整体压暗;renorm 的运动量与记忆臂同级。
2. **码本的代价可测**:31–46% 运动量损失,与事先估算的"α=1 改动 37% 向量模长"量级一致 ⇒ **FramePack 原式在免训练条件下确有可观代价**,这也是论文要求训练期替换的直接证据。
3. **先验判断被验证**:从病症(逐通道统计量漂移)推出的 renorm 优于从论文搬来的码本;被降级为对照臂的均匀量化在 rollout 上整段 std/mean 双升,符合 dither 理论证伪。
4. **边界**:n=2、只打最坏 case;运动量是动态代理不是观感质量(人眼 QC 见站点);**稳定 case 是否被弄坏尚未判**(case 0 在 wave-1 α=1 与 wave-2 α=0.5 中,指标计算中);renorm 参照取自视频自身早期窗口,若早期本身异常会锁死坏统计量。

## 2026-07-30:wave-1 完整判定 + 训练侧实现(含 renorm 训练语义的否决)

**结论 1(对照臂给出教科书式反证)**:均匀量化(quant s=0.25)在三个 case 上完全没止漂(+0.494/+0.372/+0.254 vs 基线 +0.514/+0.491/+0.034),并把**稳定 case 0 从 +0.034/峰值 108 推到 +0.254/峰值 254**。⇒ 一次性排除"任何历史扰动都有效"的平凡解释:**起作用的是复位逐通道统计量这一特定机制**。这是 dither 理论证伪的第三次独立复现(单测 → rollout 整段统计 → 逐 chunk 斜率)。
**结论 2(门槛)**:renorm 过 ①②③(失控 case 末窗 50/58 vs 194/200;稳定 case −0.015/94 vs +0.034/108;动态 88–105%),codebook 挂 ③(动态 36–66%),quantize 挂 ①②。最佳工作点 **renorm α=0.5**。完整判定:`logs/research/anti-drift-projection-verdict-2026-07-30.md`。
**结论 3(训练侧实现已就绪,但 renorm 被明确拒绝)**:投影已接入训练路径(`helios/utils/utils_helios_base.py` tier split 后、x0 拼接前,fp32 进出;`train_helios.py` 每 run 构造一次并在启动与每次存档打印计数器;`train_config.py` 四键 + 校验;50 份 YAML 补齐 no-op 默认值,`compare_yaml.py` 通过;108 tests OK)。**但 `history_projection=renorm` 在训练中被 assert 拒绝**:训练样本是来自各自视频的单窗口,不存在"同一 rollout 的更早统计量" —— per-run 参照会把整个语料的色彩往第一个样本拉(数据污染),per-sample 参照会冻结后永不投影(静默 no-op)。两条都错说明前提错,该模式要等 U>1 的 unroll 训练才有语义。训练侧可用模式:quantize / codebook(后者才是 FramePack 原式用法)。

## 2026-07-30:16 分钟视野 —— renorm 成立、记忆失效

**结论:在 2× 视野(698 chunks = 16.0 min)上,renorm α=0.5 仍把漂移压平且不损动态;而 pilot@4000 记忆臂在两个 case 中失守一个,回到基线的失败形态。**

| 臂 | case | 斜率 | 峰值 | Q4 | 动态 vs 基线 |
|---|---|---|---|---|---|
| off | 2 / 3 | +0.172 / +0.267 | 246 / 254 | 181 / 219 | 100% |
| **renorm α=0.5** | 2 / 3 | **−0.001 / −0.016** | **80 / 131** | **47 / 53** | **102% / 104%** |
| 记忆 pilot@4000 | 2 / 3 | −0.019 / **+0.177** | 168 / **255** | 74 / **164** | 103% / 93% |

1. 长视频尺度上判据要看 **Q4 与峰值**而非 Theil-Sen 斜率(序列越长、饱和越封顶,逐 chunk 斜率越小:同样两个 case 在 8min 上是 +0.514/+0.491)。
2. renorm 的 Q1 低于基线(50/61 vs 80/78)不是"画面被压暗":698 chunk 的 Q1 覆盖前 ~4 分钟,基线在这段里已开始爬升;8min 协议上 Q1 仅差 48 vs 52 可为佐证。运动量 102–104% 进一步排除压平画面。
3. **记忆的末段回拉是 case 相关的且不延伸到 16 分钟**(case 3 失守)。⇒ 记忆与投影的分工再次确认:投影防误差进入(尺度无关),记忆提供事后回复力(尺度受限)。
4. n=2,筛选级;n=5 补齐已在跑。

## 2026-07-30:renorm 的前置技术定位(CLOSE-BUT-DIFFERENT)+ 用户人眼核验

**结论:renorm 是未发表的工程技巧——数学标准、问题公认、组合未见先例。不得声称是新方法,也不得声称有同一方法的已发表引用。**

- **数学同构的先例存在**:社区 AdaIN-on-latent 节点(spacepxl `ComfyUI-Image-Filters` 的 `AdaIN Latent`、Lightricks `ComfyUI-LTXVideo` 的 `LTXVAdainLatent`)公式完全相同 `(x−μx)/σx·σy+μy`,逐通道、免训练。**差异在作用对象与参照**:它们作用于**输出**(事后颜色/风格匹配)或段间拼接,参照是用户指定或上一段;我们作用于**回灌模型的条件历史**,参照是**该 rollout 首个满员历史窗口的一次性冻结统计量**。
- **同 paradigm 的 FramePack** 诊断相同(训练/推理历史分布失配)但用码本离散化 + 训练期噪声调度,需训练。
- **症状是全领域公认的**:CausVid/Self-Forcing/Self-Forcing++/SkyReels-V2 均报告饱和度单调爬升或亮度单调下降,**但清一色在训练侧修复**(rollout-aware 蒸馏、history noise scheduling);无一采用推理期冻结统计量回灌。StreamingT2V 的 Appearance Preservation Module 在"冻结早期参照"上概念最近,但作用于可学习 CLIP 语义特征且需训练。
- **保留意见**:工程/社区空间无法穷尽搜索(约 14 次抓取),"未找到"≠"不存在"。取证:`logs/research/read-prior-art-latent-moment-renorm.md`。
- **用户人眼核验(2026-07-30)**:"非常显著的积极改善,常见的饱和度变异和过曝现象得到明显抑制" ⇒ 门槛 ④ 通过,且来自与我们指标无关的独立通道。

**对记忆主线的口径修正(重要)**:漂移不是记忆该负责的维度——它是历史通路的误差复利,有每段 32 个标量、零训练的更便宜解。记忆独有的能力是**窗口外信息保持**(19 latent 帧窗口之外的人物/背景/物体),而这至今**一次都没被量化过**(盲区①)。⇒ ① 后续记忆评测应在 renorm 打开的基础配置上进行,让记忆在它真正负责的维度上被测量;② Stage B 的验收指标必须是一致性(CLIP 轨迹 / subject consistency),而非饱和度;③ 码本训练的必要性大幅下降(每个 α 上都被 renorm 支配),除非发现码本能解决 renorm 解决不了的结构性漂移。

## 2026-07-30:chunk 边界台阶——固有 + renorm 按强度放大(用户肉眼观察被证实并定量)

**结论:用户报告的"每 1-2 秒轻微材质/颜色跳变"是真的,且由两部分构成:chunked AR 的固有边界台阶(基线饱和度边界比 1.93,最强相位 32 = 边界,排名 1/33)+ renorm 按 α 成比例的放大(α=0.125 → 3.96,α=1 → 10.47)。记忆臂不放大(1.97≈基线),codebook 放大到 6.85。**

- **逐 chunk 指标对此按构造盲**(每 33 帧一个值),必须帧级测量:`scripts/evaluation/frame_flicker_report.py`(检测器经阴性/阳性合成对照双向验证;首版把边界相位当成 0,被阳性对照抓出 off-by-one)。
- **剂量-反应关系验证了机制**:读时校正每段重算变换,滑动窗口使同一过去帧在相邻步被不同变换作用 ⇒ 边界台阶,强度越大越明显。
- **修复方向**:① `history_projection_site=write`(每 chunk 只校正一次、永久固定)从根上消除重复校正;② 降 α(0.125 只放大 2× 而止漂不变)。**基线固有的 1.93 倍不属于投影可修范围**。
- **对"是否需要训练"的判断**:renorm 的 flicker 是构造缺陷而非 OOD——α=0.125 这样极弱的干预就能完全止漂且无画质劣化,不符合分布失配的特征;应先修构造。codebook 相反:用户观察到明确画质与内容劣化,与 α=1 约 37% 模长扰动一致,训练期替换是其唯一出路。
- 判定文档:`logs/research/read-frame-flicker-chunk-boundary.md`。

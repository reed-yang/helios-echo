...
- Anti-drift history projection (inference-only, complements memory rather than replacing it): `HistoryProjector` in `helios/utils/utils_base.py` (modes `renorm` / `codebook` / `quantize`, one `alpha` knob, strict no-op by default), applied in `pipeline_helios.py` after the [long|mid|short] tier split and before the sampler so the KV cache stores the projected history; x0 anchor excluded, padding detected structurally (exactly-zero frames), effect size (`num_changed`/`max_abs_delta`) recorded so a silent no-op cannot pass as a treated arm. Codebooks from `scripts/evaluation/fit_history_codebook.py` (fit in the normalized model space the backbone consumes — corpus latents are already normalized). Design contract + paper-correspondence and choices: `docs/specs/2026-07-29-history-projection-design.md`.
- Eval toolchain (drift A/B, one driver serves all protocols): `scripts/evaluation/run_p2_interim_drift_ab.py` — `--prompt-set rep50` (structural cases, MANDATORY: raw sentences are prompt-OOD), `--segments/--sections-per-segment` (event switching via the pipeline's native interactive path), `--sections` (horizon), `--memory-partial` (load trained memory; backbone patch convs excluded by design), `--history-projection{,-alpha,-step,-codebook}`. Launchers `sbatch_p2_r3_infer.sbatch` / `sbatch_p2_r4_eventswitch.sbatch` (remap CVD to the max-free Default-mode GPU **and refuse nodes carrying pinghe's non-Slurm processes, exit 44**; optional positional args pass projection settings and widen the rep50 case window); metrics `tools/long_video_eval/scripts/run_helios_long_timeseries_metric.py`, batched by `sbatch_p2_metrics_batch.sbatch`. Judgement: `scripts/evaluation/drift_onset_report.py` (onset definitions a/b, recovery ratio, runaway and freeze flags, `--baseline` deltas) — the primary drift judgement, since the whole-run slope hides the off arm's stable/runaway bimodality. Chain evals behind training with `--dependency=afterany:<jobid>` so they survive session loss.
